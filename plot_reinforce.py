...
def plot_results(algo1, algo2):
    files_algo1 = glob.glob(f"results/reinforce_{algo1}_*.csv")
    files_algo2 = glob.glob(f"results/reinforce_{algo2}_*.csv")

    df_algo1 = pd.concat((pd.read_csv(file) for file in files_algo1))
    df_algo2 = pd.concat((pd.read_csv(file) for file in files_algo2))

    median_algo1 = df_algo1.groupby('timestep')['avg_return'].median()
    median_algo2 = df_algo2.groupby('timestep')['avg_return'].median()
    quantile_25_algo1 = df_algo1.groupby('timestep')['avg_return'].quantile(0.25)
    quantile_75_algo1 = df_algo1.groupby('timestep')['avg_return'].quantile(0.75)
    quantile_25_algo2 = df_algo2.groupby('timestep')['avg_return'].quantile(0.25)
    quantile_75_algo2 = df_algo2.groupby('timestep')['avg_return'].quantile(0.75)

    best_algo1 = df_algo1.groupby('timestep')['avg_return'].max()
    best_algo2 = df_algo2.groupby('timestep')['avg_return'].max()
    ...
    plt.title(f'Reinforce comparison with {algo1} and {algo2}')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
